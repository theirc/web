								...
								c.fields.categories && (
									<ul key="a-2">
										{c.fields.categories.map(
											a =>
												a.fields && (
													<li key={a.sys.id} onClick={() => onNavigate(`/${a.fields.slug}/${overviewOrFirst(a).fields.slug}`)}>
								...
								<ul key="a-3">
									{c.fields.articles &&
										c.fields.articles.map(
											a =>
												a.fields && (
													<li key={a.sys.id} onClick={() => onNavigate(`/${c.fields.slug}/${a.fields.slug}`)}>
														<div className="inner-container article-title">
							...
							{!showToggle(c) &&
								c.fields.overview && (
									<label
										...
										htmlFor={`tab-${i}`}
										className="container"
										onClick={() => onNavigate(`/${c.fields.slug}/${c.fields.overview.fields.slug}`)}
									>
