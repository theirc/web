...
class ArticleDetailBody extends Component {
  ...
  replaceLinks() {
    ...
    anchors = anchors.filter(
      (a) =>
        a.href.indexOf("http") ||
        a.hostname === hostname ||
        a.hostname === "www.refugee.info"
    );

    // eslint-disable-next-line
    let isPhoneOrAlreadyProcessed = (h) =>
      h.indexOf("#") === -1 &&
      h.indexOf("tel:") === -1 &&
      h.indexOf("mailto:") === -1;

    for (let anchor of anchors.filter((a) =>
      isPhoneOrAlreadyProcessed(a.href)
    )) {
  ...
  injectVideoPlaceholders() {
    ...
    Array.from(document.getElementsByClassName("YouTubePlayer") || []).forEach(
      (e) => {
    ...
    Array.from(document.getElementsByClassName("FacebookPlayer") || []).forEach(
      (e) => {
        var videoId = e.getAttribute("videoId");
    ...
    Array.from(
      document.getElementsByClassName("InstagramPlayer") || []
    ).forEach((e) => {
      var videoId = e.getAttribute("videoId");
      ReactDOM.render(
        <InstagramEmbed className={"Instagram"} url={`${IG_URL}${videoId}`} />,
        e
      );
    });
  }
  ...
  render() {
    ...
    const updatedAtDate = `${updatedDate.getFullYear()}.${appendLeadingZeroes(
      updatedDate.getMonth() + 1
    )}.${appendLeadingZeroes(updatedDate.getDate())}`;
    ...
    let categoryIcon = article?.fields?.category?.fields?.iconClass?.replace(
      "fa fa-",
      ""
    );
    return (
      <div
        ref={(r) => (this._ref = r)}
        id="articleDetailBody"
        className={["ArticleDetailBody", loading ? "loading" : "loaded"].join(
          " "
        )}
      >
        <Helmet>
        ...
        <HeaderBar
          subtitle={
            (category.fields.articles || []).length > 1 &&
        ...
        {hero?.fields?.file && (
          <div>
            <div className="hero">
              <img
                data-src={hero.fields.file.url}
                alt="hero-header"
                className="lazyload"
              />
            </div>
            {hero.fields.description && (
              <credit>{hero.fields.description}</credit>
            )}
          </div>
        )}
        ...
        <div className="ActionsBar">
          ...
          <div className="social">
            <div
              ...
              onClick={() => fbHelper.share(language)}
            >
            ...
            <div href="#" className="social-btn" onClick={this.onCopyLink}>
              ...
              {this.state.copied && (
                <span className="copied">{t("actions.Copied", NS)}</span>
              )}
            </div>
          </div>
        </div>
        ...
        <article>
          {instance.brand.url === "refugee.info" && lang.length > 0 && (
            <div id="readspeaker_button1" className="rs_skip rsbtn rs_preserve">
              <a
                ...
                href={`//app-eu.readspeaker.com/cgi-bin/rsent?customerid=11950&amp;lang=${lang}&amp;readid=articleDetailBody&amp;url=${url}`}
              >
          ...
          <div id="maincontent">
            <span className="author">
              <span>{t("actions.LAST_UPDATED", NS)}</span> {updatedAtDate}
            </span>
