...
class Services extends React.Component {
  ...
  componentWillMount() {
    ...
    if (countryRegions[0].slug !== country.fields.slug)
      countryRegions.unshift(countryData);
  ...
  fetchAllInLocation(country = "", region = "", city = "", categoryId = null) {
    ...
    const {
      sortingByLocationEnabled,
      errorWithGeolocation,
      fetchingLocation,
      geolocation,
      cities,
      regionId,
    ...
    if (city && cities && cities.length && !cityId) {
      ...
      fetchCities(regionId ? regionId : regionData.id, language)
        ...
        .then(() => {
          return this.fetchServices(
            ...
            language,
            categoryId,
            regionId ? regionId : regionData.id,
            cityId ? cityId : cityData.id
          );
        });
    }
  ...
  serviceTypes(countryWide = false) {
    ...
    if (!countryWide && regionId && !cityId) {
      return fetchCategoriesByRegion(language, regionId);
    } else if (!countryWide && cityId) {
      return fetchCategoriesByCity(language, cityId);
    }

    return fetchCategoriesByCountry(language, countryId);
  }
  ...
  goTo(region, city, location, category, mapview = false) {
    ...
    if ((!region || region.slug === country.fields.slug) && !category) {
      ...
      mapview
        ? goToRegionMap(country, region.slug)
        : listAllServicesinRegion(country, region.slug);
    } else if (region && region.slug && city && city.slug && !category) {
      mapview
        ? goToCityMap(country, region.slug, city.slug)
        : listAllServicesinCity(country, region.slug, city.slug);
    } else if (!region.slug && category) {
      mapview
        ? goToCategoryMap(country, category.id)
        : listServicesInCategory(country, category);
    } else if (region && region.slug && (!city || !city.slug) && category) {
      mapview
        ? goToRegionCategoryMap(country, category.id, region.slug)
        : goToRegionByCategory(country, category.id, region.slug);
    } else if (region && region.slug && city && city.slug && category) {
      mapview
        ? goToCityCategoryMap(country, category.id, region.slug, city.slug)
        : goToCityByCategory(country, category.id, region.slug, city.slug);
    }
  }
              ...
              <Skeleton headerColor="light">
                <div className="SkeletonContainer">
                  {isMobile && (
                    <ServiceMap
                      ...
                      findServicesInLocation={() =>
                        this.fetchAllInLocation(country.fields.slug)
                      }
                  ...
                  {!isMobile && (
                    <ServiceCategoryListDesktop
                      {...props}
                      fetchCategories={(countryId) =>
                        fetchCategoriesByCountry(
                          language,
                          countryId
                        )
                      }
                      fetchCategoriesByRegion={(regionId) =>
                        fetchCategoriesByRegion(
                          language,
                          regionId
                        )
                      }
                      fetchCategoriesByCity={(cityId) =>
                        fetchCategoriesByCity(language, cityId)
                      }
                      fetchCitiesByRegion={(regionId) =>
                        fetchCities(regionId, language)
                      }
                      fetchServices={(countryId, category, regionId, cityId) =>
                        fetchAllServices(
                          countryId,
                          language,
                          category,
                      ...
                      goTo={(
                        ...
                        this.goTo(region, city, location, category, mapview);
                ...
                <div className="SkeletonContainer">
                  {isMobile && (
                    <ServiceList
                      ...
                      showMap={() =>
                        goToCategoryMap(country, props.match.params.categoryId)
                      }
                    />
                  )}
                  {!isMobile && (
                ...
                <div className="SkeletonContainer">
                  {isMobile && (
                    <ServiceCategoryList
                      ...
                      goToMap={() =>
                        onGoToLocationMap(props.match.params.region)
                      }
                      goToLocationList={() => {
                        goToLocations(false);
                      }}
          ...
          <Route
            ...
            component={(props) => (
              <Skeleton
                headerColor="light"
                showMapButton={true}
                goToMap={() => servicesInRegionMap(country, region)}
              >
                <div className="SkeletonContainer">
                  {isMobile && (
                    <ServiceList
                      ...
                      showMap={() =>
                        goToRegionMap(country, props.match.params.region)
                      }
                    />
                  )}
                  {!isMobile && (
          ...
          <Route
            ...
            component={(props) => (
              <Skeleton
                headerColor="light"
                showMapButton={true}
                goToMap={() => servicesInCityMap(country, region, city)}
              >
                <div className="SkeletonContainer">
                  {isMobile && (
                    <ServiceList
...
const mapDispatch = (d, p) => ({
  ...
  changeDefaultLocation: (location) =>
    d(actions.changeDefaultLocation(location)),
  goToCategoryMap: (country, category) =>
    d(push(routes.goToCategoryMap(country, category))),
  goToLocation: (country, location) =>
    d(push(routes.goToLocation(country, location))),
  goToRegion: (country, region) => d(push(routes.goToRegion(country, region))),
  goToCity: (country, region, city) =>
    d(push(routes.goToCity(country, region, city))),
  goToLocationByCategory: (country, category, location) =>
    d(push(routes.goToLocationByCategory(country, category, location))),
  goToRegionByCategory: (country, category, region) =>
    d(push(routes.goToRegionByCategory(country, category, region))),
  goToCityByCategory: (country, category, region, city) =>
    d(push(routes.goToCityByCategory(country, category, region, city))),
  ...
  goToLocationMap: (country, location) =>
    d(push(routes.goToLocationMap(country, location))),
  goToRegionMap: (country, region) =>
    d(push(routes.goToRegionMap(country, region))),
  goToCityMap: (country, region, city) =>
    d(push(routes.goToCityMap(country, region, city))),
  goToLocationCategoryMap: (country, location, category) =>
    d(push(routes.goToLocationCategoryMap(country, location, category))),
  goToRegionCategoryMap: (country, category, region) =>
    d(push(routes.goToRegionCategoryMap(country, category, region))),
  goToCityCategoryMap: (country, category, region, city) =>
    d(push(routes.goToCityCategoryMap(country, category, region, city))),
  goToMap: (country) => d(push(routes.goToMap(country))),
  listAllServices: (country) => d(push(routes.listAllServices(country))),
  listAllServicesinLocation: (country, location) =>
    d(push(routes.listAllServicesinLocation(country, location))),
  listAllServicesinRegion: (country, region) =>
    d(push(routes.listAllServicesinRegion(country, region))),
  listAllServicesinCity: (country, region, city) =>
    d(push(routes.listAllServicesinCity(country, region, city))),
  listServicesInCategory: (country, category) =>
    d(push(routes.listServicesInCategory(country, category))),
  servicesInCategoryMap: (country, category, location) =>
    d(push(routes.servicesInCategoryMap(country, category, location))),
  servicesInLocationMap: (country, location) =>
    d(push(routes.servicesInLocationMap(country, location))),
